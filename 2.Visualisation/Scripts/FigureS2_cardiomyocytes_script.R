...
adult <- read.table("FigureS2_cardiomyocytes_adult.csv", sep=',', header=TRUE, row.names = 1)    
d1 <- data.matrix(adult, rownames.force = TRUE)
fetal <- read.table("FigureS2_cardiomyocytes_fetal.csv", sep=',', header=TRUE, row.names = 1)    
d2 <- data.matrix(fetal, rownames.force = TRUE)

#Renaming Columns

colnames(d2)[colnames(d2) == "Chronic.Heart.Failure"] <- "Chronic Heart Failure"
...
#Output
png("FigureS2.png", res=300, width=2500, height=1000)
