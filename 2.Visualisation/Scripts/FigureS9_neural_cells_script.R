...
adult <- read.table("FigureS9_neural_cells_adult.csv", sep=',', header=TRUE, row.names = 1)    
d1 <- data.matrix(adult, rownames.force = TRUE)
fetal <- read.table("FigureS9_neural_cells_fetal.csv", sep=',', header=TRUE, row.names = 1)    
...
colnames(d2)[colnames(d2) == "Pulse.Pressure"] <- "Pulse Pressure"
colnames(d2)[colnames(d2) == "Body.Mass.Index"] <- "Body Mass Index"
colnames(d2)[colnames(d2) == "Height"] <- "Height"
colnames(d2)[colnames(d2) == "Polycystic.Kidney.Disease"] <- "Polycystic Kidney Disease"
colnames(d2)[colnames(d2) == "Head.Injury"] <- "Head Injury"
colnames(d2)[colnames(d2) == "Height"] <- "Height"

colnames(d2)[colnames(d2) == "Back.Pain"] <- "Back Pain"
colnames(d2)[colnames(d2) == "Medication.Use.Anti.Inflammatory.And.Anti.Rheumatic.Products.Non.Steroids"] <- "Medication Use Anti Inflammatory And Anti Rheumatic Products Non Steroids"
...
#Output
png("FigureS9.png", res=300, width=3000, height=1000)
