...
adult <- read.table("FigureS11_hepatocytes_adult.csv", sep=',', header=TRUE, row.names = 1)    
d1 <- data.matrix(adult, rownames.force = TRUE)
fetal <- read.table("FigureS11_hepatocytes_fetal.csv", sep=',', header=TRUE, row.names = 1)    
...
colnames(d2)[colnames(d2) == "Total.Cholestrol.Levels"] <- "Total Cholestrol Levels"
...
colnames(d2)[colnames(d2) == "Alanine.Aminotransferase.Levels"] <- "Alanine Aminotransferase Levels"
...
#Output
png("FigureS11.png", res=300, width=2500, height=2000)
