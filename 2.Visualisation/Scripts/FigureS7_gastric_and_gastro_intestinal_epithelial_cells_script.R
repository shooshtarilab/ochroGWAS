...
adult <- read.table("FigureS7_gastric_and_gastro_intestinal_epithelial_cells_adult.csv", sep=',', header=TRUE, row.names = 1)    
d1 <- data.matrix(adult, rownames.force = TRUE)
fetal <- read.table("FigureS7_gastric_and_gastro_intestinal_epithelial_cells_fetal.csv", sep=',', header=TRUE, row.names = 1)    
d2 <- data.matrix(fetal, rownames.force = TRUE)

#Renaming Columns

colnames(d2)[colnames(d2) == "Type.1.Diabetes"] <- "Type 1 Diabetes"
...
colnames(d2)[colnames(d2) == "Serum.Uric.Acid.Levels"] <- "Serum Uric Acid Levels"
colnames(d2)[colnames(d2) == "Serum.Creatinine.Levels"] <- "Serum Creatinine Levels"
colnames(d2)[colnames(d2) == "Blood.Urea.Nitrogen.Levels"] <- "Blood Urea Nitrogen Levels"
colnames(d2)[colnames(d2) == "Calcium.Levels"] <- "Calcium Levels"
colnames(d2)[colnames(d2) == "Gamma.Glutamyl.Transpeptidase"] <- "Gamma Glutamyl Transpeptidase"
colnames(d2)[colnames(d2) == "Pulmonary.Fibrosis"] <- "Pulmonary Fibrosis"
colnames(d2)[colnames(d2) == "Gastric.Cancer"] <- "Gastric Cancer"
...
#Output
png("FigureS7.png", res=300, width=3000, height=1500)
